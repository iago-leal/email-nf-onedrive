...
> Rodadas: T001–T014 (fases 1 e 2), T015–T028 (fase 3), T029–T041 (fase 4 e ciclo completo), T043–T045 (polimento) e T042 (2026-09-21, convenção de nomes a partir da pasta real). Execução completa: 45 de 45 ações.
> Estado dos testes: 236 testes passando (`pytest`), 175 de unidade e 61 de integração, estes com IMAP dublê, Rclone real sobre o remote `:local` e Telegram dublê.
...
| `src/email_nf_onedrive/envio/nomeacao.py`, `envio.py`, `coleta/coleta.py`, `configuracao/modelo.py`, `configuracao/carregar.py`, `cli.py` | `envio-onedrive` (EO RF-01), `configuracao-caixas` | regra-alterada | MEDIUM | T042 (2026-09-21): a nomeação provisória `AAAA-MM-DD_<remetente>_<nome>` dá lugar à convenção da pasta real, `<EMPRESA> - <FORNECEDOR> [NF <n>] - <REF\|BOLETO>.<ext>`. `Caixa` ganha `empresa` (de `EMPRESA_EMAIL<n>`, padrão pelo domínio), `AnexoParaEnvio` carrega `assunto` e `classe`, e o envio lê o XML da NF-e para extrair emitente e número. Severidade média: muda o nome de todo arquivo gravado a partir de agora; arquivos já enviados não são renomeados (NG-01). |
| `docs/onedrive/*` | `envio-onedrive` §14 (OQ-01) | componente-novo | LOW | Levantamento da pasta `CONTAS A PAGAR` (análise e listagens brutas), fonte da L-03 e do cartão 2 do kanban. |
...
**`envio-onedrive`:** completo. **Nomeação (T042, 2026-09-21):** a L-03 fechou com o levantamento da pasta real; o nome passou a `<EMPRESA> - <FORNECEDOR> [NF <n>] - <REF|BOLETO>.<ext>`, em que EMPRESA vem de `EMPRESA_EMAIL<n>` (ou do domínio da caixa), FORNECEDOR do `emit/xNome` da NF-e ou do domínio do remetente (parte local quando o domínio é provedor genérico), o número de `ide/nNF` ou do primeiro `NF <n>` no nome do anexo ou no assunto, e TIPO `BOLETO` quando nome ou assunto falam em boleto. A data de recebimento saiu do nome; a rastreabilidade fica no registro (`caminho_destino`, `data_mensagem`). Colisões entre documentos distintos de mesmo fornecedor continuam resolvidas pelo sufixo `_n` (RF-04). A organização por vencimento, também observada na pasta, ficou fora desta feature. **Desvio da D-10:** a D-10, o `investigation.md` §3 e o `interfaces/rclone-onedrive.md` preveem `copyto --immutable` para impedir a sobrescrita, mas o teste com o Rclone real mostrou que `--immutable` não impede a cópia sobre um arquivo de conteúdo diferente. A fachada passou a usar `copyto --ignore-existing` e a confirmar tamanho e QuickXorHash no destino depois da cópia; divergência vira `falha-envio` com "tamanho divergente", e o arquivo da equipe nunca é substituído (verificado em `test_envio.py` e `test_ciclo_arquivamento.py`). Os dois documentos da feature ficam desatualizados nesse ponto até o `/reversa-sync`. Erros de token ou de remote inexistente suspendem os envios restantes da execução, porque todos falhariam pela mesma causa.
